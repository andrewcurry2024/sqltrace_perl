    ...
    if ( exists $spec->{can} ) {
        foreach ( ref $spec->{can} ? @{ $spec->{can} } : $spec->{can} ) {
              ...
              )
            {
                ...
                $options->{on_fail}->(
                    sprintf( "$id to $called does not have the method: '$_'\n",
                        _stringify($value) )
                );
            }
        }
...
"'callbacks' validation parameter for $called must be a hash reference\n"
    ...
    if ( exists $spec->{regex} ) {
        unless ( ( defined $value ? $value : '' ) =~ /$spec->{regex}/ ) {
            ...
            $options->{on_fail}->(
                sprintf( "$id to $called did not pass regex check\n",
                    _stringify($value) )
            );
        }
    }
...

{
    ...
    sub _typemask_to_strings {
        ...
        my @types;
        foreach (
            SCALAR,  ARRAYREF,  HASHREF, CODEREF, GLOB,
            GLOBREF, SCALARREF, UNDEF,   OBJECT,  UNKNOWN
          )
        {
    ...
    sub _get_options {
        ...
        if (@_) {
            ...
            return (
                ...
                ? { %{ $Params::Validate::OPTIONS{$caller} }, @_ }
                : { %defaults, @_ }
            );
        }
        else {
