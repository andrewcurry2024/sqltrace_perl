...
sub elide ($;$$) {
   ...
   # Leave room for the elipses and make sure we include at least one character.
    ...
    $excerpt = substr( $source, 0, $length )
      if (!defined $excerpt
        or length($excerpt) < length($source)
        and !length($excerpt)
        || abs( $length - length($excerpt) ) > $word_limit );
...
sub _expand_escape_spec {
    ...
    if ( ref($escape_spec) eq 'CODE' ) {
        ...
        return map { _expand_escape_spec($_) }
          map { $Escapes{$_} or _unsupported_escape_spec($_) }
          split( /\s+/, $escape_spec );
    }
    else {
...
sub _unsupported_escape_spec {
    ...
    croak(  "unsupported escape specification "
...
sub string2list {
    ...
    carp "string2list called with a non-text argument, '$text'"
      if ( ref $text );
