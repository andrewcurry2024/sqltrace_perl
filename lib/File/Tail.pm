...
sub READ {
    $_[0]->error(
        "READ not implemented in File::Tail -- use READLINE (<HANDLE>) instead"
    );
}

sub GETC {
    $_[0]->error(
"GETC not (yet) implemented in File::Tail -- use READLINE (<HANDLE>) instead"
    );
}
...
sub DESTROY {
    my ($this) = $_[0];
    close( $this->{"handle"} )
      if ( defined($this) && defined( $this->{'handle'} ) );
...
sub position {
    ...
    unless ( $object->{"tail"} ) {
        ...
        while ( $crs < $object->{"tail"} + 1 ) {
            ...
            $calclen = length( $object->{buffer} ) + 1024
              if int($calclen) <= length( $object->{"buffer"} );
            $calclen = $maxlen if $calclen > $maxlen;
            $object->{curpos} =
              sysseek( $object->{handle}, -$calclen, SEEK_END );
...
sub readin {
    ...
    while ( $nlen > 0 ) {
        $len =
          sysread( $object->{handle}, $object->{"buffer"}, $nlen,
...
sub read {
    ...
    unless (wantarray) {
        my $str =
          substr( $object->{"buffer"}, 0,
            1 + index( $object->{"buffer"}, "\n" ) );
        $object->{"buffer"} =
          substr( $object->{"buffer"}, 1 + index( $object->{"buffer"}, "\n" ) );
        return $str;
    }
    else {
        my @str;
